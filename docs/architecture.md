...
**HUD bar** (`HudBar.qml`, `z: 10`): index, filename, IPTC caption, star rating, EXIF date, play/pause state, key hints. `hudVisible` drives a state machine with 300 ms opacity transition. Height/fonts/margins scale with `hudScale` = `controller.hudSize / 100`. Hidden by default; toggle **I**; persisted.

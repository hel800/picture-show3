...
- `backgroundMode`: `@Property(bool, constant=True)` — True when started with `--background`; GUI starts hidden; checked in `main.qml` `onStartShow` to skip the normal show-start sequence (Python handles it instead)
